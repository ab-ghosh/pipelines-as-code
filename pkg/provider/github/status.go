...
func (v *Provider) getFailuresMessageAsAnnotations(ctx context.Context, pr *tektonv1.PipelineRun, pacopts *info.PacOpts) []*github.CheckRunAnnotation {
	...
	// Get error detection settings from repository CR if available, otherwise use global settings
	errorDetectionEnabled := pacopts.ErrorDetection
	errorDetectionMaxLines := pacopts.ErrorDetectionNumberOfLines
	errorDetectionRegexps := pacopts.ErrorDetectionSimpleRegexp
	...
	if v.repo != nil && v.repo.Spec.Settings != nil && v.repo.Spec.Settings.ErrorDetection != nil {
		...
		if repoErrorDetection.Enabled != nil {
			errorDetectionEnabled = *repoErrorDetection.Enabled
		}
		if repoErrorDetection.MaxNumberOfLines != nil {
			errorDetectionMaxLines = *repoErrorDetection.MaxNumberOfLines
		}
		if len(repoErrorDetection.SimpleRegexps) > 0 {
			errorDetectionRegexps = repoErrorDetection.SimpleRegexps
		}
	}
	...
	for _, regexStr := range errorDetectionRegexps {
		r, err := regexp.Compile(regexStr)
		if err != nil {
			v.Logger.Errorf("invalid regexp for filtering failure messages: %v", regexStr)
			continue
		}
		compiledRegexps = append(compiledRegexps, r)
	}

	if len(compiledRegexps) == 0 {
		v.Logger.Warnf("no valid regexp for error detection")
		return annotations
	}
	...
	taskinfos := kstatus.CollectFailedTasksLogSnippet(ctx, v.Run, intf, pr, int64(errorDetectionMaxLines))
	for _, taskinfo := range taskinfos {
		for _, errline := range strings.Split(taskinfo.LogSnippet, "\n") {
			// Try each regexp until one matches
			for _, r := range compiledRegexps {
				...
				// Break after first match to avoid duplicate annotations for the same line
				break
			}
		}
	}
